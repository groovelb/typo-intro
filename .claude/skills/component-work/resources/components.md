...
- ParticleFlowText: Three.js 파티클 타이포그래피. 텍스트 표면을 따라 분포된 파티클이 우측 진입→안정 드리프트→페이드 아웃 흐름. 커스텀 shader + Additive Blending (`components/kinetic-typography/ParticleFlowText.jsx`)
- measureWord (util): @chenglou/pretext measureNaturalWidth 기반 어절 폭 측정 + 캐시 (`components/kinetic-typography/measureWord.js`)
- ExpandableSentence: 순수 단락 한 문장에서 [키워드] 클릭 시 인라인 새 문장 펼침 (무한 재귀). 진짜 타자기 — transition 0, 글자 한 개씩 등장, 새 글자 폭만큼 뒷 어절이 즉시 밀림. Hermes Precisa 305 키 사운드 (use-sound + BigSoundBank CC0) (`components/kinetic-typography/ExpandableSentence.jsx`)
- useTypingSound (hook): positioned atoms 변화 관찰 → 글자 추가 시 키 사운드 (pitch ±7% 랜덤) (`components/kinetic-typography/useTypingSound.js`)
- buildIntroSentence (util): interests.json 11개 토픽을 DDD (Data Driven Design) 3 카테고리로 매핑해 ExpandableSentence 트리 생성. "내 이름은 [DDD]다" 로 시작 (`components/kinetic-typography/buildIntroSentence.js`)
